...
class Appointments_model extends EA_Model
{
    ...
    public function generate_recurring_appointments($appointment_data, $recurrence_type, $recurrence_interval, $recurrence_end_date) {
        $this->load->database();
        $start_time = new DateTime($appointment_data['start_datetime']);
        $end_time = new DateTime($appointment_data['end_datetime']);
        $recurrence_end = new DateTime($recurrence_end_date);
        $appointments = [];

        while ($start_time <= $recurrence_end) {
            if ($this->check_availability($start_time, $end_time, $appointment_data['id_users_provider'])) {
                $new_appointment = $appointment_data;
                $new_appointment['start_datetime'] = $start_time->format('Y-m-d H:i:s');
                $new_appointment['end_datetime'] = $end_time->format('Y-m-d H:i:s');
                $new_appointment['id'] = NULL; // Novo agendamento
                $new_appointment['book_datetime'] = date('Y-m-d H:i:s'); // Data de criação
                $appointment_id = $this->add($new_appointment);

                // Salvar regra de recorrência
                $this->db->insert('ea_recurring_rules', [
                    'appointment_id' => $appointment_id,
                    'recurrence_type' => $recurrence_type,
                    'recurrence_interval' => $recurrence_interval,
                    'recurrence_end_date' => $recurrence_end_date,
                    'client_id' => $appointment_data['id_users_customer'],
                    'service_id' => $appointment_data['id_services'],
                    'provider_id' => $appointment_data['id_users_provider']
                ]);

                $appointments[] = $new_appointment;
            } else {
                log_message('error', 'Conflito de horário detectado em ' . $start_time->format('Y-m-d H:i:s'));
            }

            // Avançar para a próxima data
            if ($recurrence_type == 'daily') {
                $start_time->modify("+{$recurrence_interval} days");
                $end_time->modify("+{$recurrence_interval} days");
            } elseif ($recurrence_type == 'weekly') {
                $start_time->modify("+{$recurrence_interval} weeks");
                $end_time->modify("+{$recurrence_interval} weeks");
            } elseif ($recurrence_type == 'monthly') {
                $start_time->modify("+{$recurrence_interval} months");
                $end_time->modify("+{$recurrence_interval} months");
            }
        }
    ...
    private function check_availability($start_time, $end_time, $provider_id) {
        $this->db->where('id_users_provider', $provider_id);
        $this->db->where('start_datetime <', $end_time->format('Y-m-d H:i:s'));
        $this->db->where('end_datetime >', $start_time->format('Y-m-d H:i:s'));
        $query = $this->db->get('ea_appointments');
        return $query->num_rows() == 0;
    }

    public function add(array $record): int {
        $this->db->insert('ea_appointments', $record);
        return $this->db->insert_id();
    }
